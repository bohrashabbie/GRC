...
function useCategoryTypeSchema() {
...
type FormValues = z.infer<ReturnType<typeof useCategoryTypeSchema>>
...
export function CategoryTypeFormDialog({
  ...
  const schema = useCategoryTypeSchema()
  ...
  const form = useForm<FormValues>({
    ...
    defaultValues: {
      ...
      is_active: categoryType?.is_active ?? true,
      translations: toLabelTranslationForm(categoryType?.translations),
    },
  })
  ...
  async function onSubmit(values: FormValues) {
    ...
    try {
      if (isEdit) {
        await categoryTypesApi.update(categoryType.id, {
          sort_order: values.sort_order,
          is_active: values.is_active,
          translations,
        })
      } else {
        await categoryTypesApi.create({
          sort_order: values.sort_order,
          is_active: values.is_active,
          translations,
        })
              ...
              render={({ field }) => (
                <FormItem>
                  <div className="flex items-center justify-between gap-4 rounded-lg border border-border p-3">
                    <Label htmlFor="cat-type-active">{t("fields.isActive")}</Label>
                    <FormControl>
                      <Switch
                        id="cat-type-active"
                        checked={field.value}
