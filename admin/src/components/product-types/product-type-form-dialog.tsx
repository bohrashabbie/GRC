...
type FormValues = z.infer<ReturnType<typeof useProductTypeSchema>>
...
export function ProductTypeFormDialog({
  ...
  const schema = useProductTypeSchema()
  ...
  const form = useForm<FormValues>({
    ...
    defaultValues: {
      ...
      translations: toLabelTranslationForm(productType?.translations),
    },
  })
  ...
  async function onSubmit(values: FormValues) {
    ...
    try {
      if (isEdit) {
        await productTypesApi.update(productType.id, {
              ...
              render={({ field }) => (
                <FormItem>
                  <div className="flex items-center justify-between gap-4 rounded-lg border border-border p-3">
                    <Label htmlFor="type-active">{t("fields.isActive")}</Label>
                    <FormControl>
                      <Switch
                        id="type-active"
                        checked={field.value}
